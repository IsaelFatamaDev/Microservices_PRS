    ...
    public Mono<WaterBoxTransfer> execute(String waterBoxId, String fromUserId, String toUserId,
        ...
        return waterBoxRepository.findById(waterBoxId)
                ...
                .flatMap(waterBox -> assignmentRepository.findActiveByWaterBoxId(waterBoxId)
                        .switchIfEmpty(Mono.error(new TransferNotAllowedException("No active assignment found for water box: " + waterBoxId)))
                        .flatMap(currentAssignment -> {
                            if (!currentAssignment.getUserId().equals(fromUserId)) {
                                return Mono.error(new TransferNotAllowedException(
                                        "User " + fromUserId + " is not the current assignee of water box " + waterBoxId));
                            }
                            WaterBoxAssignment deactivated = currentAssignment.markAsTransferred(createdBy);
                        ...
                        .flatMap(newAssignment -> {
                            ...
                            WaterBoxTransfer transfer = WaterBoxTransfer.builder()
                                    ...
                                    .toUserId(toUserId)
                                    .transferDate(now)
                                    .transferFee(transferFee != null ? transferFee : 0.0)
                                    .notes(notes)
                                    .recordStatus("ACTIVE")
                                    .createdAt(now)
                                    .createdBy(createdBy)
                ...
                .flatMap(saved -> eventPublisher.publishWaterBoxTransferred(waterBoxId, fromUserId, toUserId, createdBy)
                        .thenReturn(saved))
                .doOnSuccess(saved -> log.info("Water box {} transferred from {} to {} successfully", waterBoxId, fromUserId, toUserId))
                .doOnError(error -> log.error("Error transferring water box {}: {}", waterBoxId, error.getMessage()));
    }
}

...
public class PurchaseUseCaseImpl implements IPurchaseUseCase {
    ...
    public Mono<Purchase> create(Purchase purchase, String createdBy) {
        log.info("Creating purchase with code: {}", purchase.getPurchaseCode());
        return purchaseRepository.existsByPurchaseCode(purchase.getPurchaseCode())
                .flatMap(exists -> {
                    if (exists) {
                        return Mono.error(new ConflictException(
                                "Purchase with code '" + purchase.getPurchaseCode() + "' already exists"));
                    }
                    ...
                    double totalAmount = purchase.getDetails() != null
                            ? purchase.getDetails().stream()
                                    ...
                                    .sum()
                            : (purchase.getTotalAmount() != null ? purchase.getTotalAmount() : 0.0);
                ...
                .flatMap(savedPurchase -> {
                    ...
                    return Flux.fromIterable(purchase.getDetails())
                            ...
                            .collectList()
                            .map(savedDetails -> savedPurchase.toBuilder().details(savedDetails).build());
                })
                .flatMap(saved -> eventPublisher
                        .publishPurchaseCreated(saved.getId(), saved.getPurchaseCode(), createdBy)
                        .thenReturn(saved))
    ...
    public Mono<Purchase> update(String id, Purchase changes, String updatedBy) {
        ...
        return purchaseRepository.findById(id)
                ...
                .doOnError(error -> log.error("Error updating purchase {}: {}", id, error.getMessage()));
    }
    ...
    public Mono<Void> softDelete(String id, String deletedBy) {
        ...
        return purchaseRepository.findById(id)
                ...
                .doOnError(error -> log.error("Error deleting purchase {}: {}", id, error.getMessage()));
    }
    ...
    public Mono<Purchase> restore(String id, String restoredBy) {
        ...
        return purchaseRepository.findById(id)
                ...
                .doOnError(error -> log.error("Error restoring purchase {}: {}", id, error.getMessage()));
    }
        ...
        return purchaseRepository.findById(id)
                ...
                .flatMap(purchase -> {
                    ...
                    return purchaseRepository.save(receivedPurchase)
                            .flatMap(saved -> purchaseDetailRepository.findByPurchaseId(saved.getId())
                                    .flatMap(detail -> updateMaterialStockOnReceive(detail,
                                            purchase.getOrganizationId(), receivedBy))
                                    .then(Mono.just(saved)))
                            .flatMap(saved -> eventPublisher.publishPurchaseReceived(saved.getId(), receivedBy)
                                    .thenReturn(saved));
                })
                .doOnSuccess(saved -> log.info("Purchase received successfully: {}", saved.getId()))
                .doOnError(error -> log.error("Error receiving purchase {}: {}", id, error.getMessage()));
    }
    ...
    public Mono<Purchase> cancel(String id, String cancelledBy) {
        ...
        return purchaseRepository.findById(id)
                ...
                .doOnError(error -> log.error("Error cancelling purchase {}: {}", id, error.getMessage()));
    }

    private Mono<Void> updateMaterialStockOnReceive(PurchaseDetail detail, String organizationId, String receivedBy) {
        return materialRepository.findById(detail.getMaterialId())
                .flatMap(material -> {
                    Double previousStock = material.getCurrentStock() != null ? material.getCurrentStock() : 0.0;
                    ...
                    return inventoryMovementRepository.save(movement)
                            .then(materialRepository.save(material.adjustStock(newStock, receivedBy)))
                            .flatMap(updatedMaterial -> {
                                Mono<Void> stockEvent = eventPublisher.publishStockUpdated(
                                        material.getId(), previousStock, newStock, receivedBy);

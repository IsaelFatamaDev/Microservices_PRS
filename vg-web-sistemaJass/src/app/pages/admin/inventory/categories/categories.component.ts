    ...
    <div class="p-6">
      ...
      <div class="bg-white rounded-xl shadow-sm p-4 mb-4">
        <div class="relative">
          ...
          <input [(ngModel)]="searchTerm" (ngModelChange)="applyFilters()" type="text" placeholder="Buscar categorías..." class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-violet-500/20 placeholder:text-gray-300">
        </div>
      </div>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" (click)="closeModal()">
          <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full" (click)="$event.stopPropagation()">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 class="text-lg font-bold text-gray-800">{{ editMode() ? 'Editar' : 'Nueva' }} Categoría</h3>
            </div>

            <div class="p-6 space-y-3">
              <div>
                <label class="text-sm text-gray-600 mb-1 block">Nombre de la Categoría *</label>
                <input [(ngModel)]="formData.categoryName" type="text" placeholder="Ej: Materiales de Construcción" class="w-full px-3 py-2 border border-gray-200 rounded-xl placeholder:text-gray-300 outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400">
              </div>

              <div>
                <label class="text-sm text-gray-600 mb-1 block">Descripción</label>
                <textarea [(ngModel)]="formData.description" rows="3" placeholder="Descripción de la categoría..." class="w-full px-3 py-2 border border-gray-200 rounded-xl resize-none placeholder:text-gray-300 outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400"></textarea>
              </div>
            </div>
            ...
            <div class="flex gap-3 px-6 pb-6">
              ...
              <button (click)="save()" [disabled]="!isFormValid()" class="flex-1 px-4 py-2.5 bg-violet-600 hover:bg-violet-700 text-white font-medium rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
...
export class CategoriesComponent {
  ...
  delete(id: string) {
    this.alert.confirmDelete('esta categoría').then(result => {
  ...
  restore(id: string) {
    this.alert.confirmRestore('esta categoría').then(result => {
      if (!result.isConfirmed) return;
